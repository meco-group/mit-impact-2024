...
# This function simulates the controlled quadcopter
...
def set_options():
    ...
    return ctrlType, trajSelect

def simulate(ctrl, traj, quad, wind, Ts, Ti, Tf, start_time):
    ...
    simulation_results = {"t_all":t_all, "pos_all":pos_all, "vel_all":vel_all, 
                          "quat_all":quat_all, "omega_all":omega_all, 
                          "euler_all":euler_all, "w_cmd_all":w_cmd_all, 
                          "wMotor_all":wMotor_all, "thr_all":thr_all, 
                          "tor_all":tor_all, "sDes_traj_all":sDes_traj_all,
...
def main():
    ...
    Ti = 0
    Ts = 0.005
    Tf = 20 # will be updated later according to the optimal trajectory
    ifsave = 0

    # Choose trajectory settings
    # ---------------------------
    ctrlType, trajSelect = set_options()

    # Initialize Quadcopter, Controller, Wind, Result Matrixes
    # --------------------------------------------------------
    quad = Quadcopter(Ti)
    # traj = Trajectory(quad, ctrlType, trajSelect)
    traj = OptimalTrajectory(quad, ctrlType)
    ctrl = Control(quad, traj.yawType)
    wind = Wind('None', 2.0, 90, -15)
    # wind = Wind('SINE', 2.0, 90, -15)
    # wind = Wind('RANDOMSINE', 5.0, 3.0, 90, 120, -15, 0.0)
    ...
    # Run Simulation
    # ---------------------------
    simulation_results = simulate(ctrl, traj, quad, wind, Ts, Ti, Tf, start_time)


    # View Results
    # ---------------------------
    # utils.fullprint(sDes_traj_all[:,3:6])
    utils.makeFigures(quad.params, simulation_results)
    ani = utils.sameAxisAnimation(simulation_results, traj.wps, Ts, quad.params, traj.xyzType,
                                  traj.yawType, ifsave, traj.obstacle_centers, 
                                  traj.obstacle_radii)
    plt.show()
